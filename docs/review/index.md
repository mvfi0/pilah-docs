# Individual Review

Evidence for the individual review (IR), organised by competency. Each page states the competency level and the dated evidence behind it, with commit and PR links.

## Part B — Hardskills: Best Practice

Part B is assessed weekly, so evidence on each page is grouped by week.

| Competency | Level | Status (22 Sep) |
|---|---|---|
| [Test Driven Development](part-b/tdd.md) | **3** | Evidence complete |
| [Programming](part-b/programming.md) | **3** | Evidence complete |
| [Development Discipline](part-b/development-discipline.md) | **2** (15–21 Sep) · **3** (22–28 Sep) | Evidence complete |
| [Team Development Management](part-b/team-development-management.md) | **2–3** | Reviewed pilah-be #22 |
| [Code Quality](part-b/code-quality.md) | **2–3** | 3 SonarQube issues still to check |
| [Security](part-b/security.md) | **1** | OWASP items not yet named in commits |
| [AI Literacy](part-b/ai-literacy.md) | **3** | Evidence complete |

Main work so far: [PIL-176](https://linear.app/pilah-2/issue/PIL-176) (pencatatan pencairan): backend [pilah-be #19](https://github.com/bank-sampah-PILAH/pilah-be/pull/19) and mobile [pilah-mobile #26](https://github.com/bank-sampah-PILAH/pilah-mobile/pull/26).
