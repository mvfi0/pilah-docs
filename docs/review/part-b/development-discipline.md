...
!!! success "Competency level: 3"
    Two merge requests in week 1, one per repository, 41 commits in scoped Conventional Commits, each MR with a detailed description and green CI before review was requested.
...
- **Branch** follows the Linear-linked convention, the same name in both repos, so Linear links each PR to its issue automatically: `feature/pil-176-pencatatan-pencairan-untuk-pengurus`.
...
- **Worked in an isolated worktree** following the team's `ship` workflow.
...
style(pencairan): dart format repository test
```

PIL-222 (riwayat pencairan) is claimed in [week 2](../../review-week-2/index.md), where its merge requests were reviewed and revised.
